...
async def main():
    ...
    async for message in client.iter_messages(TELEGRAM_CHANNEL, limit=None):
        if message.date.replace(tzinfo=None) < start_of_year:
            print("올해 1월 1일 이전 메시지이므로 백필을 종료합니다.")
            break
        ...
        if found_company and message.document and message.document.mime_type == 'application/pdf':
            print(f"[{found_company}] 리포트 발견! ({message.date.strftime('%Y-%m-%d')})", flush=True)
