...
if ticker == '425420':
    ...
    st.markdown("""
    #### 1. 핵심 투자 포인트: "ISC의 SKC 피인수에 따른 삼성전자 벤더 다변화 수혜"
    * **배경**: 경쟁사 ISC가 SK그룹(SKC)에 인수됨에 따라, 삼성전자는 보안 및 경쟁사 견제를 위해 테스트 소켓 핵심 벤더를 다변화해야 하는 강한 니즈가 발생했습니다.
    * **결과**: 티에프이가 삼성전자 내 메모리(1c 나노 등) 및 AP 비메모리 테스트 소켓 점유율을 적극적으로 흡수하고 있습니다.
    
    #### 2. 가장 중요한 선행 지표 (원재료 매입액)
    소켓과 보드 부품의 **'원재료 분기 매입액'**은 1~2개 분기 뒤의 **'매출액 폭발'**을 예고하는 가장 확실한 선행 지표입니다.
    * **BOARD PARTS 매입액**: 평소 분기당 7~14억 수준이던 매입액이 25년 1분기, 2분기에 **연속 34억**으로 폭증했습니다. 
    * **PCB 매입액**: 1억 미만이던 매입액이 25년 상반기에 **27억, 18억**으로 수직 상승했습니다.
    * **소켓 원재료 (수입/내수)**: 역대급 매입을 기록 중입니다.
    👉 **결론**: 상반기에 쓸어 담은 원재료들이 제품으로 조립되어 출하되는 **3분기~4분기 역대급 실적(Q의 폭발)**을 강력하게 시사합니다.

    #### 3. 지속 트래킹 방법 (어디서 확인해야 하는가?)
